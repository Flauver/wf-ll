...
local processor = rime.make_option_cycler(options,
  'll/spelling/cycle_key',
  'll/spelling/switch_key')
...
local function xform(s)
  -- input format: "[spelling·code_code...·pinyin_pinyin...·CJK·U+XXXX]"
...
local function spell_phrase(s, spll_rvdb)
  ...
  return table.concat(radicals, " ")  -- 添加空格分隔
end
...
local function code_phrase(s, spll_rvdb)
  ...
  for i, coord in ipairs(rule) do
    ...
    -- 若任一取码单字没有注解数据，则不对词组作注。
    if raw == '' then return end
    local code_str = parse_raw_code_comment(raw)
    -- 编码部分可能包含多个编码，用下划线分隔
    local char_codes = {}
    for code in code_str:gmatch('[^_]+') do
      table.insert(char_codes, code)
    end
    local code_idx = coord[2] > 0 and coord[2] or #char_codes + 1 + coord[2]
    codes[i] = char_codes[code_idx] or '～'
  end
  return table.concat(codes, " ")  -- 添加空格分隔
end
...
local function get_tricomment(cand, env, spll_rvdb)
  ...
  if utf8.len(text) == 1 then
    ...
    if env.engine.context:get_option('ll_spelling.lv1') then
      -- 只显示拆分（第一个元素）
      return xform(raw_spelling:gsub('%[(.-)·.*%]', '[%1]'))
    elseif env.engine.context:get_option('ll_spelling.lv2') then
      -- 显示拆分和编码（前两个元素）
      return xform(raw_spelling:gsub('%[(.-)·(.-)·.*%]', '[%1·%2]'))
    else
      -- ll_spelling.lv3 is on - 显示所有元素
  ...
  elseif utf8.len(text) > 1 then
    ...
    if code and code ~= '' then
      -- 不再需要分割和排序，因为 code_phrase 已经返回用空格分隔的编码
...
local function filter(input, env)
  if env.engine.context:get_option('ll_spelling.off') then
    for cand in input:iter() do yield(cand) end
    return
  end
  local spll_rvdb = nil
  local code_rvdb = nil
  if env.engine.context:get_option('ll_spelling_source') then
...
local function init(env)
  local spll_rvdb = env.engine.schema.config:get_string('ll/spelling/schema')
  local code_rvdb = env.engine.schema.config:get_string('ll/code')
  local spll_rvdb_tw = env.engine.schema.config:get_string('ll/spelling_tw/schema')
  local code_rvdb_tw = env.engine.schema.config:get_string('ll/code_tw')
